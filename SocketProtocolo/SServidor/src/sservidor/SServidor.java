...
public class SServidor {
    ...
    private static ConcurrentMap<PrintWriter, Set<String>> sentMessages = new ConcurrentHashMap<>();

    public static void main(String[] args) {
        try (ServerSocket serverSocket = new ServerSocket(PORT)) {
            System.out.println("El servidor de chat está en línea en el puerto " + PORT);
    ...
    private static class ClientHandler extends Thread {
        ...
        public void run() {
            try {
                ...
                sentMessages.put(writer, new HashSet<>());

                BufferedReader reader = new BufferedReader(new InputStreamReader(socket.getInputStream()));

                // Leer el primer dato y guardarlo en la variable CIFRADO
                String cifrado = reader.readLine();
                System.out.println("CIFRADO: " + cifrado);

                // Leer el segundo dato y guardarlo en la variable CLAVE
                String clave = reader.readLine();
                System.out.println("CLAVE: " + clave);
            String mensajeEncriptado = cifrado;
       
            String codiencri = clave;
            ...
            System.out.println("Mensaje desencriptado: " + Descenriptado);
                ...
                while ((message = reader.readLine()) != null) {
                    ...
                    // Verificar si el mensaje ya ha sido enviado a este cliente
                    if (!sentMessages.get(writer).contains(message)) {
                        broadcastMessage(message);
                        sentMessages.get(writer).add(message);
                    }
                }
            } catch (IOException e) {
                e.printStackTrace();
